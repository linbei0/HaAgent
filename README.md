...
- **先规划再执行**：用 `/plan` 进入只读规划模式，确认或反馈方案后自动生成 Todo 并继续执行。
...
需要先审阅方案时，输入 `/plan <任务>`；也可以先输入 `/plan`，再在输入框中描述任务。规划阶段只允许读取和分析，不会修改文件或执行命令。方案出现后可直接提交反馈要求修订，或切换到批准按钮开始执行。批准后 HaAgent 会把步骤转换为 Todo：窄终端显示当前进度摘要，宽终端可展开查看 `待处理`、`进行中`、`已完成` 和 `已取消` 四种状态。规划确认或执行过程中按 `Ctrl+X` 会取消任务；仅按 `Esc` 只收起确认面板，不会默认批准或拒绝。
