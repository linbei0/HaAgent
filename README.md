...
- **模型配置中心**：通过 profile 管理 OpenAI Responses-compatible 和 OpenAI Chat Completions-compatible endpoint。
...
- 支持 provider：`openai` 与 `openai-chat`
- API key 解析顺序：当前环境变量、系统凭据库、显式 opt-in 的明文用户文件

推荐在 TUI 的 `/model` 中配置和切换模型。TUI 只展示环境变量名、凭据来源和 key 是否可用等非敏感状态，不显示、复制或写入真实 API key。

一个最小 profile 形状如下，`api_key_env` 指向环境变量名，不要把真实 key 写入仓库：
