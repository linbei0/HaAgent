"""
haagent/runtime/dogfood.py - 真实模型 dogfood 运行器
    ...
    return [
        _DogfoodTask(
            ...
            prompt=(
                ...
                "先用 context_find 定位相关上下文，再 file_read，最后用 apply_patch_set 一次完成相关修改。"
            ),
...
def _verify_howdy_workspace(workspace: Path, tool_calls: list[dict[str, Any]]) -> tuple[bool, str]:
    ...
    if "context_find" not in _tool_names(tool_calls) or "apply_patch_set" not in _tool_names(tool_calls):
        return False, "expected context_find and apply_patch_set"
    return True, "none"
...
def _task_improvement(
    ...
    if status == "completed":
        return "none"
    if name == "context-edit" and "context_find" not in tools:
        return "prompt/schema should make context_find the obvious first move"
