...
更新说明：本文保留 2026-06-27 的历史审计材料。当前实现已采纳后续产品决策：无子命令 `haagent` 默认进入 TUI，TUI 是唯一普通交互入口；旧 `setup/chat/sessions/memory/tui` 交互命令只提示迁移。本文中“不要切默认入口”的判断不再作为当前规则，当前规则以 `docs/harness-requirements.md` 和 `docs/code-governance.md` 为准。
...
历史建议曾认为不应立即把 `haagent` 默认入口切到 TUI；该建议已被新的产品决策取代。当前方向是 TUI 作为普通入口，继续把它打磨成“个人助手会话工作台”的可靠前端。
...
- 普通路径是在任意目录运行 `haagent` 并进入 TUI；模型配置在 TUI 内 `/model` 完成。
- TUI 已成为普通默认路径。
...
lazygit、Posting、Yazi 这类成熟 TUI 的共同点是：核心区域位置稳定，用户能形成“某类信息总在某处”的记忆。HaAgent 当前以主对话区、状态栏和上下文弹窗为核心，但任务状态仍需要更稳定的呈现，例如：
...
当前布局有框架，但信息还需要在不增加界面负担的前提下组织得更清楚。
...
- `/` 搜索。
- `:` 或 `Ctrl+P` 命令面板。
- slash commands，例如 `/sessions`、`/resume`、`/new`。
- `@file` 文件引用。
- `Shift+Tab` 反向焦点。
- 当前模式下的详细帮助 overlay。
...
成熟 Agent CLI/TUI 往往有 slash commands 或命令面板，例如 `/help`、`/sessions`、`/memory`、`/settings`。HaAgent 当前把功能绑定在少量按键上，短期够用，长期不利于扩展。

建议采用“双入口”：

- `Ctrl+P` 或 `:` 打开 Textual command palette，用于发现和执行 UI 命令。
- 输入框内 `/` 开头作为 Agent/TUI 命令，例如 `/sessions`、`/new`、`/resume`、`/memory`、`/clear`。
...
3. 工具进度和修改摘要不足。
   ...
   - 建议：对话区保留简短工具状态行，失败块指向 episode trace；文件修改摘要另行设计，不复用右侧工作台。
...
2. 响应式策略需要约束正文宽度。
   - 建议：增加 80x24 最小门槛、窄屏可读布局、超宽不继续拉长正文。
...
- 增加 `/` 搜索当前对话。
- 增加命令面板或 slash command 的最小集合：`/help`、`/sessions`、`/memory`、`/new`、`/resume`。
...
- 对话区工具状态行保持简短，失败块提供阶段、原因和 episode path。
...
2. 不建议用自然语言匹配实现 slash commands 或安全边界。
   - HaAgent 的目标是本地个人助手。文件树、编辑器、多标签工作台可以作为辅助能力，但不应成为主界面。

3. 不建议为了视觉效果大量加入装饰。
   - 命令、工具、session、workspace 都应走结构化 service 方法和明确状态字段。

4. 不建议把完整 stdout、patch、episode trace 默认塞进主对话。
...
- 120x40 下能同时看到对话、状态和 session 摘要。
...
- Textual `run_test()` 覆盖核心交互，至少包含 80x24、120x40、多候选、session overlay 和帮助 modal。
...
这条路线能最大限度复用当前已有的 `AssistantService` 和测试基础，也符合项目“不增加普通用户心智负担、把 TUI 作为唯一普通交互入口持续打磨”的约束。
