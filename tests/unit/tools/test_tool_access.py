...
def _resolve(requested: list[str], *, mcp=None):
...
def test_process_tools_remain_visible_and_are_controlled_by_tool_approval() -> None:
    snapshot = _resolve(["file_read", "shell", "code_run"])

    assert snapshot.allowed_tools == ("file_read", "shell", "code_run")


def test_mcp_tools_require_connected_runtime() -> None:
    unavailable = _resolve(["list_mcp_resources"], mcp=_Mcp("failed"))
    connected = _resolve(["list_mcp_resources"], mcp=_Mcp("connected"))
...
def test_unsupported_vision_removes_attachment_tool() -> None:
