"""
tests/test_cli_run.py - HaAgent run 子命令测试
...
def test_cli_parser_accepts_chat_request_with_defaults() -> None:
    parser = cli.build_parser()

    args = parser.parse_args(["chat", "List this project"])

    assert args.command == "chat"
    assert args.request == "List this project"
...
def test_cli_chat_fake_provider_workflow_writes_task_and_reports_unverified(
    tmp_path: Path,
    capsys,
    monkeypatch,
) -> None:
    ...
    exit_code = cli.main(["chat", "List project", "--provider", "fake"])
    ...
    assert task.allowed_tools == ["file_list", "file_search", "file_read", "apply_patch", "shell"]
    assert task.verification_commands == []
    assert task.policy["approved_tools"] == ["apply_patch", "shell"]


def test_cli_chat_defaults_workspace_root_to_current_directory(
    tmp_path: Path,
    monkeypatch,
) -> None:
    monkeypatch.chdir(tmp_path)
    ...
    class FakeOrchestrator:
        def __init__(self, runs_root: Path, model_gateway=None, max_turns: int = 3) -> None:
            calls["runs_root"] = runs_root
            calls["model_gateway"] = model_gateway
            calls["max_turns"] = max_turns

        def run(self, received_task_path: Path) -> FakeResult:
            calls["task"] = load_task(received_task_path)
            return FakeResult(tmp_path / ".runs" / "episode-1")

    monkeypatch.setattr(cli, "RunOrchestrator", FakeOrchestrator)

    exit_code = cli.main(["chat", "Summarize this workspace"])

    assert exit_code == 0
    assert calls["runs_root"] == Path(".runs")
    assert calls["max_turns"] == 20
    assert calls["task"].workspace_root == str(tmp_path.resolve())


def test_cli_chat_accepts_explicit_workspace_root(
    tmp_path: Path,
    monkeypatch,
) -> None:
    ...
    exit_code = cli.main(["chat", "Work here", "--workspace-root", str(workspace)])

    assert exit_code == 0
    assert calls["max_turns"] == 20
    assert calls["task"].workspace_root == str(workspace.resolve())
...
def test_cli_chat_profile_does_not_leak_secret(
    ...
    output = capsys.readouterr().out
    assert exit_code == 0
    assert calls["api_key"] == "sk-secret-value-that-must-not-leak"
    assert calls["model"] == "chat-test"
    assert calls["base_url"] == "https://api.example/v1"
    assert calls["max_turns"] == 20
    assert "sk-secret-value-that-must-not-leak" not in output
    assert "sk-secret-value-that-must-not-leak" not in calls["task_text"]
